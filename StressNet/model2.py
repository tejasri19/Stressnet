...
class MDCN(nn.Module):
    def __init__(self, in_channels):
        ...
        self.downsample = nn.Sequential(
            nn.Conv2d(in_channels, 64, kernel_size=5, padding=0, stride=2),
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 128, kernel_size=5, padding=0, stride=2),
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.Conv2d(128, 128, kernel_size=5, padding=0, stride=2),
            nn.ReLU(),
            nn.BatchNorm2d(128)
            #nn.Conv2d(128, 256, kernel_size=5, padding=0, stride=2),
            #nn.ReLU()
        )
        #self.conv1 = nn.Conv2d(128, 128, kernel_size=5, padding=0, stride=1)
...
class ARCNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.lstm = nn.LSTM(input_size=self.embedding_size, hidden_size=256,
                            num_layers=1, batch_first=True, bidirectional=True)

        # Fully connected layers
        self.fc1 = nn.Linear(2*256, 16)
